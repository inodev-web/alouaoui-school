    ...
    try {
      ...
      if (response && response.data) {
        ...
        setStats({
          ...
          todayProgrammed: sessions.filter(s => s.status_raw === null || s.status_raw === undefined).length,
          todayCancelled: sessions.filter(s => s.status_raw === 'cancelled').length,
          todayCompleted: sessions.filter(s => s.status_raw === 'completed').length,
        })
      }
    } catch (error) {
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-right">الجلسات</CardTitle>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground text-right">إجمالي الجلسات</p>
          </CardContent>
        </Card>

        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-right">بانتظار التأكيد</CardTitle>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground text-right">لم يتم تأكيدها بعد</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground text-right">تمت بنجاح</p>
          </CardContent>
        </Card>

        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-right">ملغاة</CardTitle>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground text-right">تم إلغاؤها</p>
          </CardContent>
        </Card>
      </div>
