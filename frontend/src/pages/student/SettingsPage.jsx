...
export default function StudentSettingsPage() {
  ...
  const submitProfile = async (e) => {
    e.preventDefault();
    if (!canModify) { setError('لقد قمت بتعديل المعلومات اليوم، الرجاء المحاولة غداً'); return; }
    ...
    try {
      ...
      fd.append('current_password', passwords.current_password);
  ...
  const submitPassword = async (e) => {
    ...
    setLoading(true); setError(null); setSuccess(null);
    try {
      if (!passwords.current_password) { setError('كلمة المرور الحالية مطلوبة'); setLoading(false); return; }
      if (passwords.password !== passwords.password_confirmation) { setError('تأكيد كلمة المرور غير مطابق'); setLoading(false); return; }
  ...
  return (
    <div dir="rtl" className="max-w-5xl mx-auto p-6 space-y-8">
      ...
      <Card>
        <CardHeader><CardTitle>تعديل المعلومات الشخصية</CardTitle></CardHeader>
        <CardContent>
          <form onSubmit={submitProfile} className="grid gap-4 md:grid-cols-2">
            ...
            <div className="md:col-span-2 space-y-1">
              ...
              <Input type="password" name="current_password" value={passwords.current_password} onChange={handlePassChange} />
            </div>
            <div className="md:col-span-2 flex justify-end">
              <Button type="submit" disabled={loading || !canModify}>{loading ? 'جاري الحفظ...' : 'حفظ التعديلات'}</Button>
            </div>
            {!canModify && <p className="text-xs text-yellow-600 md:col-span-2">لقد قمت بتعديل بياناتك اليوم، الرجاء المحاولة بعد 24 ساعة.</p>}
          </form>
        </CardContent>
      </Card>
        ...
        <CardContent>
          <form onSubmit={submitPassword} className="grid gap-4 md:grid-cols-2">
            ...
            <div className="space-y-1">
              ...
              <Input type="password" name="password_confirmation" value={passwords.password_confirmation} onChange={handlePassChange} />
            </div>
            <div className="md:col-span-2 flex justify-end">
              <Button type="submit" disabled={loading}>{loading ? 'جاري الحفظ...' : 'تأكيد التغيير'}</Button>
            </div>
          </form>
        </CardContent>
      </Card>
