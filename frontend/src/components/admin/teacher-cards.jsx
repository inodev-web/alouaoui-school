...
import { Users, DollarSign, BookOpen, Star, Phone, Calendar } from "lucide-react"
...
export function TeacherCards() {
  ...
  const getAvatar = (teacher) => {
    ...
    const initials = (teacher.name || '?').split(' ').map(p => p[0]).join('').substring(0,2)
  ...
  if (error) {
    return (
      <div className="py-10 text-center" dir="rtl">
        ...
        <Button onClick={loadTeachers} className="bg-indigo-600 hover:bg-indigo-700">إعادة المحاولة</Button>
      </div>
    )
  }
    ...
    <div className="space-y-6" dir="rtl">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2 items-center">
          <Badge variant="outline" className="text-sm px-3 py-1">المجموع: {teachers.length}</Badge>
      ...
      <div className="grid gap-5 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
        {teachers.map((t) => (
          <Card key={t.uuid} className="shadow-md border-0 bg-gradient-to-br from-white via-indigo-50 to-blue-50 hover:shadow-lg transition">
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  <img
                    ...
                    onError={(e) => { e.target.src = getAvatar(t) }}
                  />
                  <div>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-white rounded-lg p-3 flex items-center gap-2 border border-blue-100">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-bold text-gray-800">{t.studentsCount ?? 0}</p>
                  </div>
                </div>
                {t.percent_school && (
                  <div className="bg-white rounded-lg p-3 flex items-center gap-2 border border-green-100">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold text-gray-800">{t.percent_school}%</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              {(t.price_subscription || t.price_session) && (
                <div className="space-y-1 bg-gradient-to-r from-amber-50 to-orange-50 rounded-lg p-3 border border-amber-200">
                  ...
                  {t.price_subscription && (
                    <p className="text-xs text-gray-700">الإشتراك الشهري: <span className="font-bold text-green-700">{t.price_subscription} دج</span></p>
                  )}
                  {t.price_session && (
                    <p className="text-xs text-gray-700">سعر الحصة: <span className="font-bold text-indigo-700">{t.price_session} دج</span></p>
                  )}
                </div>
              )}
              ...
              <div className="flex flex-col gap-2 text-xs text-gray-600">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span>تاريخ الإضافة: {new Date(t.created_at).toLocaleDateString('ar-DZ')}</span>
                </div>
              </div>
            </CardContent>
          </Card>
