    ...
    if (onSave) {
      try {
        ...
        if (!savedCourse) {
          ...
          console.warn('No course data returned by onSave, but creation may have succeeded. Response:', response)
          onOpenChange(false)
          return
        }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <DialogHeader>
          ...
          <div id="course-form-description" className="text-sm text-muted-foreground">
        ...
        <form onSubmit={handleSubmit}>
                  ...
                  <div className="mt-2 p-2 bg-blue-50 rounded-lg border border-blue-200">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="text-sm font-medium text-blue-800">{formData.pdf_summary.name}</p>
                          <p className="text-xs text-blue-600">
                            {(formData.pdf_summary.size / 1024 / 1024).toFixed(2)} MB
                          </p>
                        </div>
                      </div>
                      <Button 
                  ...
                  <div className="mt-2 p-2 bg-green-50 rounded-lg border border-green-200">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="text-sm font-medium text-green-800">{formData.exercises_pdf.name}</p>
                          <p className="text-xs text-green-600">
                            {(formData.exercises_pdf.size / 1024 / 1024).toFixed(2)} MB
                          </p>
                        </div>
                      </div>
                      <Button 
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={uploading}>
              {uploading ? 'جاري الحفظ...' : course ? 'حفظ التغييرات' : 'إضافة الدرس'}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
